...
procedure onReadResponse:
    ...
    def var i-qtd-bytes-read-aux as int no-undo.
    def var i-qtd-bytes-avail-aux as int no-undo.
...
procedure doReadResponse private:
    ...
    assign i-qtd-bytes-avail-aux = self:get-bytes-available().
    
    // can't ensure the response is finished so...
	// takes a little timeout to await
    if i-qtd-bytes-avail-aux > 1024
    then assign i-qtd-bytes-avail-aux = 1024.
    else pause 0.001 no-message.
    
    set-size(m-temp-aux) = 0.
    set-size(m-temp-aux) = i-qtd-bytes-avail-aux.
    set-byte-order(m-temp-aux) = big-endian.
    self:read(m-temp-aux, 1, i-qtd-bytes-avail-aux, read-available).

    assign i-qtd-bytes-read-aux = self:bytes-read.

    set-size(m-swap-aux) = 0.
    set-size(m-swap-aux) = get-size(m-response-aux) + i-qtd-bytes-read-aux.
